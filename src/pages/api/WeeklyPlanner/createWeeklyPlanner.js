...
const createWeeklyPlanner = async (req,res) =>{
    try{
        ...
        const weekly_plannerCreated = await prisma.weekly_planner.create({
            data: weekly_planner
        })

    if(weekly_plannerCreated){
        res.status(200).json({success:true, message: "Weekly planner creato", data: weekly_plannerCreated});
    }else{
        res.status(500).json({errore:"Problema nella creazione del weekly planner "+e });
    }
